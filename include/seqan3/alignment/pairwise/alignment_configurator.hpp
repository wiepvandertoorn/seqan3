...
private:
    ...
    using first_seq_t  = std::tuple_element_t<0, std::iter_value_t<std::ranges::iterator_t<unref_range_type>>>;
    //!\brief The type of the second sequence.
    using second_seq_t = std::tuple_element_t<1, std::iter_value_t<std::ranges::iterator_t<unref_range_type>>>;
    //!\}
...
public:
    ...
    constexpr static bool expects_tuple_like_value_type()
    {
        return tuple_like<alignment_config_type> &&
               std::tuple_size_v<std::iter_value_t<std::ranges::iterator_t<unref_range_type>>> == 2;
    }
    ...
    constexpr static bool expects_valid_scoring_scheme()
    {
        if constexpr (alignment_config_type::template exists<align_cfg::scoring>())
        {
            ...
            return static_cast<bool>(scoring_scheme<scoring_type,
                                                   std::iter_value_t<first_seq_t>,
                                                   std::iter_value_t<second_seq_t>>);
        }
        else
        {
            return false;
